...
class ConsolidatedWizards(models.TransientModel):
    ...
    is_todos_company = fields.Selection([("uno", "A Company"), ("todos", "All the Companies")], "Select Companies", help="To select Companies", default="todos", required=True)
    company_id = fields.Many2one("res.company", string="Company", default=lambda self: self.env.user.company_id, required=True)

    currency_id = fields.Many2one("res.currency", string="Currency", default=70, required=True)
