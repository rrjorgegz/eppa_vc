...
class ConsumptionIndexCompaniesReport(models.AbstractModel):
    _name = "report.l10n_cu_eppa.consumption_index_companies"
    _description = "Consumption Index for Companies"
    ...
    def _get_report_values(self, docids, data=None):
        ...
        query_init = """SELECT t0.nombre_comp,t0.nombre_prod_unit,t0.nombre_ingrediente, AVG(t0.indice_consumo) AS indice_consumo,AVG(t0.indice_consumo_plan) AS indice_consumo_plan,SUM(t0.product_uom_qty) AS consumo_real, SUM(t0.consumo_plan)AS consumo_plan, (SUM(t0.product_uom_qty) - SUM(t0.consumo_plan)) AS exceso,(SUM(t0.product_uom_qty) /nullif(SUM(t0.consumo_plan),0)*100) AS cumplimiento,SUM(t0.produccion) FROM
                    ...
                    GROUP BY t0.nombre_comp,t0.nombre_prod_unit,t0.nombre_ingrediente
                            ...
                            for m in range(len(matrix_data)):  # recorrer datos para el hijo unidad
                                if x_padre_prod_unit[k][0] == matrix_data[m][1]:
                                    x1_hijo = (
                                        ...
                                        matrix_data[m][8], matrix_data[m][9])
        ...
        t1 = ""
        if len(matrix_sql) > 0:
            sql = """SELECT t0.dato0 AS nombre_comp,t0.dato1 AS nombre_prod_unit,t0.dato2 AS nombre_ingrediente, AVG(t0.dato3) AS indice_consumo,AVG(t0.dato4) AS indice_consumo_plan,SUM(t0.dato5) AS consumo_real, SUM(t0.dato6)AS consumo_plan, (SUM(t0.dato5) - SUM(t0.dato6)) AS exceso,(SUM(t0.dato5) /nullif(SUM(t0.dato6),0)*100) AS cumplimiento ,SUM(t0.dato9) AS produccion FROM
                    (SELECT t1.dato0,t1.dato1,t1.dato2,t1.dato3,t1.dato4,t1.dato5,t1.dato6,t1.dato7,t1.dato8,t1.dato9
                    FROM (VALUES """ + matrix_sql + """)
                    t1(dato0,dato1,dato2,dato3,dato4,dato5,dato6,dato7,dato8,dato9)) AS t0
                    GROUP BY nombre_comp,nombre_prod_unit,nombre_ingrediente
                    ORDER BY nombre_prod_unit
                    """
            t1 = """SELECT t5.nombre_prod_unit, SUM(t5.produccion) FROM
        ...
        for unit in prod_unit_id:
            for prod in docs_prod:
                if prod[0] == unit.name:
                    conj_unit.append(prod)
