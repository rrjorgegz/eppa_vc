...
class ConsolidatedReport(models.AbstractModel):
    ...
    def _get_report_values(self, docids, data=None):
        ...
        f2 = datetime(year=end.year, month=end.month, day=end.day, hour=23, minute=59, second=59, microsecond=999999, tzinfo=None).strftime("%Y-%m-%d %H:%M:%S")
        ...
        query = (
            """SELECT t2.company_id,t2.company ,t2.category_id,t2.category,t2.currency_id ,t2.currency,t2.product_template,t2.produccion_uf,
        ...
        INNER JOIN res_currency AS ru ON ru.id = cep.currency_id AND ru.id =  """
            + currency
            + """
        ...
        query2 = (
            """SELECT  product_template FROM ("""
            + query
            + """) AS t3  GROUP BY product_template
         """
        )
        self.flush()
        ...
        for i_matriz in i1:
            for dato in x:
                if dato[3] == i_matriz["category"] and dato[6] == i_matriz["bom"] and dato[1] == i_matriz["company"]:
        ...
        for i_dato in i1:
            ...
            for buscando_dato in i1:
                if i_dato["company_id"] == buscando_dato["company_parent_id"] and i_dato["category"] == buscando_dato["category"] and i_dato["bom"] == buscando_dato["bom"]:
        ...
        for c in company_id:
            ...
            d.append(company)
        moneda = self.env["l10n_cu_mrp.commercialization"].search([("currency_id", "=", data["form"]["currency_id"])])
