...
class ConsumptionIndexIngredientsReport(models.AbstractModel):
    ...
    def get_mrp_prods_ids_from_prod_unit(self, prod_unit_main, filters):
        mrp_prods_ids = []
        mrp_prods_ids = mrp_prods_ids + self.env['mrp.production'].search(
            [("prod_unit_id", "=", prod_unit_main.id), ("date_planned_start", ">=", filters["start"]),
             ("date_planned_start", "<=", filters["end"]), ("company_id", "in", filters["company_id"].ids)]).ids
        for child in prod_unit_main.child_ids:
            if len(child.child_ids) > 0:
                ...
                mrp_prods_ids = mrp_prods_ids + self.env['mrp.production'].search(
                    [("prod_unit_id", "=", child.id), ("date_planned_start", ">=", filters["start"]),
                     ("date_planned_start", "<=", filters["end"]), ("company_id", "in", filters["company_id"].ids)]).ids
    ...
    def get_array_unit_mrp_prod_from_prod_unit(self, prod_unit_main, filters):
        ...
        for child in prod_unit_main.child_ids:
            if len(child.child_ids) > 0:
                ...
                array_unit.append(
                    {"prod_unit_id": child.id, "mrp_prod": self.get_mrp_prods_ids_from_prod_unit(child, filters)})
        return array_unit
    ...
    def get_info_per_prod_unit(self, unit_data, filters):
        ...
        for d in productos_ids:
            ...
            aux_mrp_prods = self.env['mrp.production'].search(
                [('id', 'in', unit_data["mrp_prod"]), ('product_id', '=', d.id),
                 ('product_id', 'in', filters["product_id"].ids)])
        ...
        for ingrediente_group in ingredientes_ids:
            ...
            raw_ing = self.env['stock.move'].search(
                [('id', 'in', moves_ids), ('product_id', '=', ingrediente_group.id)])
            ...
            for x3 in raw_ing:
                ...
                count += 1
                produccion_kg += x3.raw_material_production_id.produccion
            ...
            if ingrediente_group in filters['ingredient']:
                raw_ingredient = {"ing": ingrediente_group.product_tmpl_id.name, "produccion": produccion_kg,
                                  "indice_consumo": ic, "indice_consumo_plan": icp, "consumo": co, "consumo_plan": cop,
                                  "exceso": exc, "cumplido": cum}
    ...
    def get_ingredientes_in_move_raw_ids(self, move_raw_ids):
        ...
        for x2 in move_raw_ids:
            if not x2.product_id in ingredientes_ids:
        ...
        for unit1 in mrp_produccion_unit_prod:
            if unit1["prod_unit_id"] in filters["prod_unit_id"].ids:
                ...
                for mrp_p1 in aux_mrp_p1:
                    if mrp_p1.bom_id.forma_c == filters["commercialization_id"] and mrp_p1.bom_id.mrp_dep_id == filters[
                        "departament_id"]:
