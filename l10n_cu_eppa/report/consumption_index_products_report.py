...
class ConsumptionIndexProductsReport(models.AbstractModel):
    ...
    def redondeo(self, cantidad):
        return round(float(cantidad),
                     int(self.env['decimal.precision'].search([('name', '=', 'Product Unit of Measure')]).digits))
    ...
    def _get_report_values(self, docids, data=None):
        ...
        query4 = """SELECT  product,produccion  FROM (""" + query + """) AS tabla1 GROUP BY product,produccion """
        self.flush()
        ...
        query4 = """SELECT partner,partner_parent_id, partner_name,SUM(produccion),company FROM (""" + query + """) AS tabla1 GROUP BY partner,partner_parent_id,partner_name,company ORDER BY partner DESC"""
        self.flush()
        self.env.cr.execute(query4)
            ...
            if aux_a[0] in company:
                ...
                for aux_c in aux_4:
                    ...
                    for aux_b in x4:
                        b = {}
                        b['product'] = aux_b[0]
                        b['produccion'] = aux_b[1]
                        b['datos'] = []
                        for aux_d in i:
                            if aux_d['partner'] == aux_c[0] and aux_a[1] == aux_d['company'] and aux_b[0] == aux_d['product'] and float(
