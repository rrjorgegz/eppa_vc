...
class ConsumptionIndexProductsReport(models.AbstractModel):
    ...
    def get_child_production_unit(self, n):
        if len(n.child_ids) == 0:
            return n
        for i in n.child_ids:
            n = n | self.get_child_production_unit(i)
        return n
    ...
    def _get_report_values(self, docids, data={}):
        ...
        dat = []
        for i in prod_unit_id:
            mps = (
                self.env["mrp.production"]
                .search(
                    [
                        ("prod_unit_id", "in", self.get_child_production_unit(i).ids),
                        ("date_planned_start", ">=", data["form"]["start"]),
                        ("date_planned_start", "<=", data["form"]["end"]),
                        ("product_id", "in", product_id.ids),
                        ("company_id", "in", company_id.ids),
                    ]
                )
                .move_raw_ids.filtered(lambda r: r.product_id.id in ingredient_tmpl_id.ids)
                .filtered(lambda r: r.product_id.mrp_dep_id.id in departament_id.ids)
            )
            # .mapped(i.name,'product_id.name')
            # forma_c
            dat.append(mps)
        # x = dat.sort()
        print(dat[0].mapped("name"))
...
#                         {"ing": ingrediente_group.product_tmpl_id.name, "indice_consumo": ic, "indice_consumo_plan": icp, "consumo": co, "consumo_plan": cop, "exceso": exc, "cumplido": cum}
#                     )
