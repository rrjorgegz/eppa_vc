...
class ScaleGroup(models.Model):
    ...
    currency = fields.Many2one('res.currency', 'Currency', required=1, groups="base.group_multi_currency",
                               readonly=False, default=70)

    _sql_constraints = [
        ('name_uniq', 'unique (name)', 'The name of the Scale Group must be unique!')
    ]