...
class MrpBom(models.Model):
    ...
    forma_c = fields.Many2one("l10n_cu_mrp.commercialization", string="Commercialization", help="Form of Commercialization", index=1, required=True)
    ...
    concepto_count = fields.Integer("# Concept of Expenses", compute="_compute_concepto_gastos_count", compute_sudo=False)
    ...
    def _compute_salario_obrero_count(self):
        for salario in self:
            ...
            if salario.id:
                salario.salario_count = self.env["l10n_cu_mrp.wage_worker"].search_count([("mrp_bom_id", "=", salario.id)])

    def _compute_concepto_gastos_count(self):
        for concepto in self:
            concepto.concepto_count = 0
            if concepto.id:
                concepto.concepto_count = self.env["l10n_cu_mrp.concept_expenses"].search_count([("mrp_bom_id", "=", concepto.id)])

    @api.onchange("product_qty", "product_uom_id", "product_tmpl_id", "weight")
    def get_production(self):
        self.produccion = self.product_uom_id._compute_quantity(qty=self.product_qty, to_unit=self.product_uom_id, raise_if_failure=False) * self.weight
        return
