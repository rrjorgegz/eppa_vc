...
class MrpProduction(models.Model):
    ...
    produccion = fields.Float(string="Production", digits="Product Unit of Measure", default=0)
    weight = fields.Float(string="Weight", related="product_tmpl_id.weight", digits="Product Unit of Measure", default=0)
    ...
    def get_produccion(self):
        self.produccion = self.weight * self.product_uom_id._compute_quantity(qty=self.product_qty, to_unit=self.product_uom_id, raise_if_failure=False)
        return
    ...
    def move_raw_ids_consumo_plan(self):
        ...
        for raw in self.move_raw_ids:
            ...
            raw.indice_consumo_plan = (self.bom_id.bom_line_ids.search([("bom_id", "=", self.bom_id.id), ("product_tmpl_id", "=", raw.product_id.product_tmpl_id.id)], limit=1).indice_consumo) or 0
            raw.consumo_plan = (
                self.produccion * ((self.bom_id.bom_line_ids.search([("bom_id", "=", self.bom_id.id), ("product_tmpl_id", "=", raw.product_id.product_tmpl_id.id)], limit=1).product_qty) or 0)
